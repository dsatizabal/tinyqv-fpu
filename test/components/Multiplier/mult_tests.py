...
def float_to_half_bits(f):
    """Converts Python float to IEEE-754 half-precision bits."""
    return int(np.float16(f).view(np.uint16)) & 0xFFFF

def half_bits_to_float(bits):
    """Converts 16-bit int to Python float using IEEE-754 half-precision."""
...
async def apply_and_wait(dut, a_float, b_float):
    ...
    await Timer(10, units='ns')  # Simulate a small request pulse
    ...
    result_float = half_bits_to_float(result_bits)

    # Lower request to reset internal FSM
    dut.req_in.value = 0
    await Timer(100, units='ns')  # Give time to reset

    return result_float

@cocotb.test()
async def test_async_fpu_mul_normal(dut):
    dut.req_in.value = 0
    dut.a.value = 0
    dut.b.value = 0
...
@cocotb.test()
async def test_async_fpu_mul_with_signs(dut):
    dut.req_in.value = 0
    dut.a.value = 0
    dut.b.value = 0
    await Timer(100, units='ns')
...
@cocotb.test()
async def test_async_fpu_mul_edge_cases(dut):
    dut.req_in.value = 0
    dut.a.value = 0
    dut.b.value = 0
    await Timer(100, units='ns')
